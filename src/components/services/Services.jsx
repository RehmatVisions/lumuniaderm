...
export default function Services() {
  ...
  return (
    <section id="services" className="relative overflow-hidden bg-[#0c0806] py-20 lg:py-28">
      ...
      {/* ── Atmospheric bg — static blobs (no animate = zero CPU) ── */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -left-48 top-16 h-[700px] w-[700px] rounded-full bg-novaderm-gold/[0.045] blur-[130px]" />
        <div className="absolute -right-48 bottom-16 h-[600px] w-[600px] rounded-full bg-novaderm-brown/60 blur-[110px]" />
        <div className="absolute left-[28%] top-[35%] h-[380px] w-[380px] rounded-full bg-novaderm-gold/[0.028] blur-[90px]" />
      </div>

      <div className="relative mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-10">

        {/* ── Section header ── */}
        <div className="mb-14 grid gap-6 sm:grid-cols-2 sm:items-end lg:mb-20">
          <div className="flex flex-col gap-4">

            {/* badge */}
            <motion.span
              className="inline-flex w-fit items-center gap-2 rounded-full border border-novaderm-gold/30 bg-novaderm-gold/[0.08] px-4 py-1.5"
              {...fadeUp(0)}
            >
            ...
            <motion.h2
              className="text-3xl font-semibold leading-tight tracking-tight text-white sm:text-4xl lg:text-[2.5rem] lg:leading-[1.14]"
              {...fadeUp(0.08)}
            >
              {headline}
            </motion.h2>
            ...
            {/* gold underline */}
            <motion.div
              className="h-px w-20 bg-gradient-to-r from-novaderm-gold to-transparent"
          ...
          <motion.div className="flex flex-col items-start gap-4 sm:items-end" {...fadeUp(0.14)}>
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {cards.map((card, i) => {
            ...
            return <Component key={card.title} card={card} index={i} />
          })}
        </div>

        {/* ── Bottom stats bar ── */}
        <motion.div
          className="mt-14 grid grid-cols-2 gap-4 rounded-[1.5rem] border border-white/[0.06] bg-white/[0.025] p-6 backdrop-blur-sm sm:grid-cols-4 sm:p-8"
        ...
        >
          {[
            ...
            <motion.div key={stat.label} className="flex flex-col items-center gap-1 text-center"
              initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.15 + i * 0.07 }}>
